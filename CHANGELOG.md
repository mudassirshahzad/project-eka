...
Two operational-integrity gaps that both live at the Weaviate boundary: a retrieval call that could hang forever, and ingestion state that could silently diverge between Postgres and Weaviate. Sequenced deliberately **before** re-ranking (WP-4), so that work does not add a second external call to a retrieval path whose first one is still unbounded.

- **Weaviate client timeouts — ADR HD03 closed after three releases (ADR OR01)** — a hand-constructed `WeaviateClient` bean now bounds connect / connection-request / read timeouts (`app.weaviate.*-timeout-seconds`, defaults 5/5/20), replacing the auto-configured client that inherited the library's 60-second defaults. HD03 investigated this at P05.5 and documented precisely why it could not be fixed then (no timeout property, no customization hook) and what a fix would require ("overriding the auto-configured client and hand-constructing `io.weaviate.client.Config`") — which is exactly what this does. Both enabling facts were re-verified against the actual jars rather than assumed: the auto-configured bean carries `@ConditionalOnMissingBean`, and `Config`'s own two-argument constructor passes `60, 60, 60`, proving the unit is **seconds**
- **Units are seconds, deliberately breaking the `*-timeout-ms` convention (ADR OR01)** — a millisecond-to-second conversion would introduce a rounding step where a sub-second value silently becomes `0`, and `0` means *unbounded* in this client: precisely the failure the bean exists to remove. A non-positive timeout now fails the context at startup (ADR EX04 precedent)
- **Postgres↔Weaviate reconciliation (ADR OR02)** — `ChunkReconciliationService` finds chunks committed to Postgres whose vectors never reached Weaviate and repairs them. A null `vector_id` is not a heuristic but the definition of that drift: `Chunk.assignVectorId` runs only after indexing succeeds, so a committed row without one means indexing did not complete — a state that became reachable when ADR HD01 correctly replaced one long transaction with short per-step ones. Repair re-runs the **real** ingestion collaborators (`EmbeddingService` → `DocumentIndexingService`, already idempotent) rather than a reconciliation-specific path that would be free to drift from the one it checks. Bounded per pass, oldest first, each chunk repaired independently so one failure cannot abort the batch
- **Alerting is a staleness gauge, not just counters (ADR OR03)** — `eka.reconciliation.seconds_since_last_success` grows until a pass completes and is deliberately *not* reset by a failed pass. Counters alone cannot express the failure that hides every other one: a job that has silently stopped running reports no drift, exactly like a healthy system. One alert rule (`> 2 × interval`) now covers both a stalled scheduler and a dependency outage, while `eka.reconciliation.failed` separately surfaces drift that was detected but could not be repaired
- **Deliberately not covered** — Weaviate-side orphans (a vector with no surviving Postgres chunk). Detecting them properly needs a full-collection enumeration `VectorStore` does not expose, and the exposure is low: the delete path deletes vectors first and is idempotent (ADR HD07), so the worst residue is an unreferenced vector no query can surface, since every retrieval result is resolved back through Postgres. Recorded as a bounded limitation rather than left unstated

### Fixed

- **A real ingestion-path bug, found by this work package's first real-database test (ADR OR04)** — `ChunkRepositoryAdapter.saveAll` mapped *every* chunk to a fresh detached entity, including already-persisted ones. Since `created_at` is `updatable = false`, the value survived in the database but the returned instance came back with `createdAt = null`, which `Chunk.reconstitute` rejects. This is the ingestion path's normal second save (`DocumentIndexingService` re-saves chunks to record the `vectorId` indexing produced), not an edge case — it had simply never been exercised against a real database, the same coverage shape ADR R03 described when the first real-database persistence test uncovered a genuine schema bug. It surfaced here because reconciliation's repair re-saves chunks by design, so the bug would have broken repair exactly when it was needed. `saveAll` now updates the existing row, copying only the state indexing changes
- ADRs OR01–OR04 frozen (see `.claude/DECISIONS.md`)
- **775 total tests, 0 failures** (net +15): `ChunkReconciliationServiceTest` (new, 8) — no-drift path never touches the ingestion collaborators, detection and repair, one failing chunk not aborting the batch, counters, batch bounding, constructor guards; `WeaviateClientConfigTest` (new, 3) — client construction with and without an API key, and non-positive timeouts rejected at startup; `ChunkReconciliationQueryIT` (new, 4) against real Postgres — a simulated partial write detected, limit bounding, and the `createdAt`-preserving re-save regression. ArchUnit: 8/8, no new layering violations
...
- **Repository prerequisite found by running it, not by assuming it** — GitHub's Dependency Graph was *disabled* on this repository despite it being public, which makes every GitHub-native SCA mechanism fail. The first CI run surfaced it honestly: the submission job held the correct `Contents: write` permission and still failed, with `dependency-graph/sbom` and `vulnerability-alerts` both returning `404`. Enabling Dependabot vulnerability alerts turned the graph on (`sbom` → `200`) and the re-run went green. Recorded in ADR CG01 because nothing in the workflow file expresses it and a fork — or a future platform consumer reusing this CI pattern — will hit the identical failure against an already-correct workflow
...
Second milestone of Phase 6 (frozen roadmap, ADR GOV03) — closes the item this project's target architecture has named since before Phase 4 ("Authorization Filter (planned)... do not assume it already exists") and the literal Phase 6 exit criterion ("retrieval results are provably scoped by document-level authorization in a test, not just by tenant"). Three design inputs this milestone needed (classification hierarchy, role→clearance mapping, null-classification transition handling) had no prior specification anywhere in the codebase and were resolved as explicit decisions before implementation began.

- **Classification hierarchy** — new `DocumentClassification` enum: `PUBLIC` < `INTERNAL` < `CONFIDENTIAL` < `RESTRICTED`, carrying an explicit numeric `level` (never `Enum.ordinal()`, so a future tier can be inserted without renumbering every tier below it). `DocumentMetadata.classification` is now this enum, not an unvalidated free `String` — no persistence schema change needed, it round-trips through the existing `VARCHAR(50)` column exactly as `format`/`status` already do (ADR AF01)
- **Role → clearance mapping** — a fixed, in-code table (`RoleBasedClassificationPolicyAdapter`): `VIEWER`→`INTERNAL`, `AUDITOR`/`USER`→`CONFIDENTIAL`, `ADMIN`→`RESTRICTED`. A caller's clearance is the maximum across every `UserRole` they hold, consistent with this codebase's existing multi-role support (ADR AF02)
- **Single shared policy component** — new domain port `ClassificationPolicyPort` (+ its sole adapter) is the one place clearance is decided; both `RetrievalService` (retrieval pipeline) and `DocumentApplicationService` (every REST document endpoint) call it, so the two access paths can't drift into different authorization rules (ADR AF04)
- **Retrieval-pipeline enforcement, engine-agnostic** — `RetrievalService` resolves and filters by classification once, after `RetrievalPort.retrieve(...)` returns (keyed off `RetrievedChunk.documentId()`, present regardless of which engine produced a result), not pushed into `MetadataFilter`/either adapter's native filter translation. This was a real finding during implementation, not a simplification of choice: `HybridRetrievalAdapter` shares one `MetadataFilter` across the Weaviate and BM25 sub-adapters, and Weaviate indexes no per-chunk classification property — pushing a classification criterion into that shared filter would have silently zeroed out *all* vector-search results the moment it was populated (ADR AF03)
- **Consistent enforcement across every document REST endpoint** — `getDocument`/`deleteDocument` now resolve to the same `404` (never a distinct `403`) a genuinely nonexistent or wrong-tenant document already returns; `listDocuments`/`listDocumentsByOwner` push the caller's maximum clearance level into the listing query itself (`DocumentJpaRepository`'s new JPQL `CASE`-predicate queries), enforced before `LIMIT`/`OFFSET` so pagination stays correct — not a post-fetch filter, which would silently corrupt `PageResult.totalElements`/page size (ADR AF05)
- **Classification now mandatory at ingestion** — `POST /api/v1/documents` rejects a missing or unrecognized `classification` value with `400`; `Document.create` independently rejects a null classification (defense in depth); `Document.reconstitute` stays unvalidated, mirroring the exact precedent ADR EX08 set for `Conversation`'s title invariant (ADR AF06)
- **Fail-closed transition** — `V018__backfill_null_document_classification.sql` (idempotent by construction) classified every pre-existing document `INTERNAL`; the migration and this enforcement code ship in the same release, and Flyway migrations complete before any request is served, so there is exactly one runtime behavior for a null/unknown classification — deny, via `DocumentClassification.UNKNOWN_LEVEL` — never a separate "pre-backfill" code path. New `UnclassifiedDocumentStartupCheck` (`ApplicationRunner`) logs a `WARN` with a count if any document is still unclassified at startup — operational visibility, not the enforcement mechanism itself (ADR AF07)
- ADRs AF01–AF07 frozen (see `.claude/DECISIONS.md`)
- 53 new tests: `DocumentClassificationTest` (new file, 10) — level ordering, `parse`/`levelOf` for known/null/blank/unrecognized values, `UNKNOWN_LEVEL` always highest; `RoleBasedClassificationPolicyAdapterTest` (new file, 12) — per-role clearance, multi-role union, empty-role-set denial, null-classification/unknown-classification denial even for `ADMIN`, full role×tier matrix; `UnclassifiedDocumentStartupCheckTest` (new file, 2); `DocumentApplicationServiceTest` (new file, 7) — permitted/denied classification for get/delete (denial → `ResourceNotFoundException`, never a distinct exception type), clearance level pushed into list queries; `GetDocumentUseCaseTest`/`ListDocumentsUseCaseTest` (new files, 4/6) — null-guards including the new `roles` parameter; `RetrievalServiceTest` (+7) — constructor guards for the two new dependencies, null-roles guard, a denied chunk dropped before ranking, an entirely-denied result skips ranking, a chunk whose document no longer exists is denied; `DocumentControllerTest` (+2) — missing/invalid classification on upload both return `400`; `RagEndToEndIT` (+3) — a real `RESTRICTED` document denied to a `VIEWER` (`404`) and readable by an `ADMIN` (`200`) against a real Postgres row, a real listing query excluding an above-clearance document from a real paginated page. 726 total tests, 0 failures. ArchUnit: 8/8, no new layering violations.

## [0.7.0] — 2026-08-19 — Product Completeness & Authorization Depth — REST Surface Foundation

### Added (P06.1 — Product Completeness & Authorization Depth — REST Surface Foundation)

First milestone of Phase 6 (frozen roadmap, ADR GOV03) — not Phase 6 in full; the Authorization Filter (P06.2) has not started. Begins exposing existing internal capabilities through REST: document ingestion, first-user bootstrap and minimal user administration, and the two conversation operations (list, delete) that already had tested use cases but no route. No architecture redesign — every endpoint reuses an existing port, application service, or use case unchanged; the only new domain-adjacent code is the tenant-isolation fix two of those use cases needed once this milestone made them reachable.

- **Document ingestion REST API** — new `DocumentController` (`/api/v1/documents`): `POST` (multipart upload, `USER`/`ADMIN`), `GET /{id}` and `GET` (list, tenant-wide, any authenticated role — matches `DocumentApplicationService`'s existing tenant-scoped-not-owner-scoped semantics), `DELETE /{id}` (`USER`/`ADMIN`). Reuses `UploadDocumentUseCase`/`GetDocumentUseCase`/`ListDocumentsUseCase`/`DeleteDocumentUseCase` exactly as they already existed — no ingestion business logic added. Closes the post-Phase-5 audit's finding H2 and the v0.6.1-deferred decision (ADR EX09) to build it later. Format is detected from filename first, then MIME type, before falling back to a 400 (ADR PC05)
- **Request size filter narrowed, not widened, to support uploads** — `RequestSizeLimitFilter`'s 1 MiB `Content-Length` limit (v0.6.1, ADR EX06) would have rejected any real document upload; self-review caught that exempting by content type alone would let a request to *any* endpoint claim to be multipart and get the much larger 100 MB multipart limit instead. Fixed to require an exact match on method + path + content type — only `POST /api/v1/documents` is exempt, not "anything multipart" (ADR PC01)
- **First-user bootstrap and minimal admin surface** — new `AdminController` (`/api/v1/admin`): public `POST /bootstrap` (creates the first `ADMIN` user for an already-provisioned, still-empty tenant — safe to leave public because `UserApplicationService.tenantHasAnyUser` rejects every call after the first), `POST /users` (`ADMIN`-only, registers an additional user in the caller's own tenant), `GET /users/{id}` and `POST /users/{id}/deactivate` (`ADMIN`-only). Deliberately minimal — no list-users, no role-assignment, no password-change REST surface; those application-service methods already work but had no concrete requirement pulling them into this milestone (ADR PC05). No `Tenant` domain aggregate or repository port was introduced — tenant creation stays an ops/database concern, exactly as before; a bootstrap call against a genuinely nonexistent tenant surfaces as a generic 400 via a new `DataIntegrityViolationException` handler, not a domain-level 404, a disclosed trade-off of not building that port for this alone (ADR PC03)
- **Closed a real cross-tenant gap in `UserApplicationService`** — `getUser`/`activateUser`/`deactivateUser` fetched by bare `UserId` with no tenant check at all; `activateUser`/`deactivateUser` already accepted a `tenantId` parameter but never verified it. Harmless while unreachable, but this milestone is what makes `getUser` reachable for the first time (the new admin `GET /users/{id}`) — fixed with the same `requireTenantMatch`-after-fetch pattern `ConversationApplicationService` already established (ADR TN01/HD02): a mismatch resolves to 404, never a distinct "forbidden." `GetUserUseCase.execute` gained a `tenantId` parameter to match (ADR PC02)
- **Conversation list and delete** — `ConversationController` gained `GET /` (paginated, own conversations only, open to every authenticated role) and `DELETE /{id}` (`USER`/`ADMIN`, calls `DeleteConversationUseCase` — kept, not deleted, at v0.6.1 specifically for this day, ADR EX08). `DeleteConversationUseCase`'s active-chat-session guard is exercised by a real caller for the first time
- **Exception handling gap closed** — a bare `ApplicationException` (the base type `RegisterUserUseCase`'s validation and `DeleteConversationUseCase`'s active-session guard already threw) had no handler of its own and fell through to the generic 500. `GlobalExceptionHandler` gained `ApplicationException` → 400 (Spring's most-specific-match resolution leaves `ResourceNotFoundException`/`DuplicateResourceException`/`InvalidCredentialsException` unaffected) and `DataIntegrityViolationException` → 400 (ADR PC04)
- **`PageResponse<T>`** — new shared paginated-list response DTO (`api.dto`), used by both `DocumentController.listDocuments` and `ConversationController.listConversations` — introduced once two real callers needed it in the same milestone, not speculatively
- ADRs PC01–PC05 frozen (see `.claude/DECISIONS.md`)
- 50 new tests: `DocumentControllerTest` (new file, 13) — upload success/empty-file/unsupported-format/auth/role checks, get/list/delete including tenant-scoped-not-owner-scoped read; `AdminControllerTest` (new file, 12) — bootstrap success/already-initialized/validation, register/get/deactivate role checks; `UserApplicationServiceTest` (new file, 6) and `GetUserUseCaseTest` (new file, 3) — the new tenant-check behavior; `ConversationControllerTest` (+12) — list, delete (including the active-session-guard-now-400 case), role/auth checks; `RequestSizeLimitFilterTest` (+2, one changed) — the path-scoped multipart exemption, and proof a multipart-content-typed request to an unrelated path is still capped. `RagEndToEndIT` (+6) — real bootstrap-then-login password-hash round trip, real "second bootstrap call rejected" against a real Postgres row, real cross-tenant document-read 404, real tenant-wide document readability, real delete-then-404 for conversations, real list-conversations ownership scoping — all against Testcontainers Postgres, not mocks. 673 total tests, 0 failures. ArchUnit: 8/8, no new layering violations.

## [0.6.1] — 2026-08-16 — Engineering Excellence & Repository Governance
