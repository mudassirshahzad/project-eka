...
✅ v0.8.5 — Security Remediation & Platform Enablement (2026-09-26). A track running **alongside** the frozen Phase 6 → 7 → 8 sequence, not inside it: it closes nothing in Phase 7 and claims nothing of Phase 8. 103 of 104 dependency advisories closed (8/8 critical, 40/40 high; ADR DEP01–DEP03), EKA published and consumed as a library jar with a `platform-smoke` consumer test in CI (ADR PL01–PL06), and `UserDetailsServiceAutoConfiguration` excluded (ADR SEC01). No new platform capability and no Phase 7 criterion closed.

Upcoming (frozen sequence, not yet started)

◐ Phase 7 — Retrieval Quality & Operational Integrity. **All five work packages shipped; the phase is NOT closed** — one exit criterion remains open (retrieval-quality criterion, ADR RQ07). Branch protection was applied on `main` on 2026-09-26 and verified live (ADR GOV10), closing the first of ADR GOV09's two rows. Re-ranking, HyDE, Postgres↔Weaviate reconciliation, refresh tokens, Weaviate client timeout, prompt-injection review, SCA in CI, applied branch protection. **Planned in its own session (plan approved); implementation in progress.** Scope is unchanged from ADR GOV03 — the plan groups it into five work packages by engineering boundary, each with its own point release (ADR GOV07): WP-1 CI/CD & Release Governance Hardening → v0.8.0 (shipped); WP-2 Session Security (refresh tokens + revocation) → v0.8.1 (shipped); WP-3 Operational Resilience (Weaviate timeout + reconciliation) → v0.8.2 (shipped); WP-4 Retrieval Quality (re-ranking + HyDE + evaluation harness) → v0.8.3 (shipped; quality success criterion left OPEN — ADR RQ07); WP-5 Prompt-Injection Review + Phase 7 Complete gate → v0.8.4 (shipped; **gate returned NOT COMPLETE** — ADR GOV09). Applying branch protection stayed a repository-owner action executed outside the implementation session; it was verified unapplied at the v0.8.4 gate (ADR GOV08) and has since been applied (ADR GOV10). Full work-package tracking lives in `.claude/PROJECT_STATE.md`.
